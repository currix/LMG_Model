PROGRAM avalavec_modelH_LIPKIN
  ...
  !
  INTEGER(KIND = I4B) :: state_index, state_index_2, mval
  !
  !
  ! NAMELISTS
  ...
  ENDIF
  !
  ! Only y_par = 0 case. TODO y_par non zero
  IF (y_par /= 0) STOP 'y_par non zero error'
  !
  ...
  !modham_param(3) => 2Jz (Perturbation to break degeneracy)
  ...
  !
  ! ALLOCATE BASIS
  ALLOCATE(SO2_Basis(1:dim_block), STAT = IERR)    
  IF (IERR /= 0) THEN
     WRITE(UNIT = *, FMT = *) "SO2_Basis allocation request denied."
     STOP
  ENDIF
  !
  CALL SO2_BASIS_VIBRON(N_val, SO2_Basis) ! Build SO2 basis
  !
  ...
  !
  Ham_mat = 0.0_DP
  CALL Build_Mod_Ham_SO2(N_val, dim_block, SO2_basis, Ham_mat) 
  !
  !
  IF (Iprint > 2) THEN
     WRITE(*,*) ' '
  ...
  DO state_index = 1, dim_block
     !
     !mval = SO2_Basis(state_index)%m_SO2_val
     !
     !
     IF (MXZ_Log) THEN
        CALL magnetization_XZ_SO2(N_val, dim_block, Ham_mat(:, state_index), MX, MZ)
        WRITE(UNIT = *, FMT = *) state_index, Eigenval_vector(state_index)/REAL(N_val,DP), MX/N_val, MZ/N_val
     ELSE
        WRITE(UNIT = *, FMT = *) state_index, Eigenval_vector(state_index)/REAL(N_val,DP)
     ENDIF
     !
     !
     IF (Eigenvec_Log .AND. Iprint > 0) THEN
        ...
        DO state_index_2 = 1, dim_block
           !
           mval = SO2_Basis(state_index_2)%m_SO2_val
           !
           WRITE(UNIT = *, FMT = *) Ham_mat(state_index_2, state_index), "|",mval,">"
           !
        ENDDO
        !
     ENDIF
  ...
  IF (Save_avec_Log) CALL SAVE_EIGENV_COMPONENTS(N_val, dim_block, "so2", Ham_mat)
  !
  !    
  ...
  ENDIF
  ! DEALLOCATE BASIS
  DEALLOCATE(SO2_Basis, STAT = IERR)    
  IF (IERR /= 0) THEN
